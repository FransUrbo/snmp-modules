...
# {{{ $Id: bind9-snmp-stats.pl,v 1.7 2005-10-12 06:34:34 turbo Exp $
...
sub echo {
	...
	print LOG &get_timestring()," $string";
    }
}
# }}}
...
# }}} # OID_BASE.2
	...
	my @tmp = split('\.', $oid);
	&output_extra_debugging("=> .3.1: ", @tmp) if($DEBUG > 2);
...
# }}} # OID_BASE.3
	...
	my @tmp = split('\.', $oid);
	&output_extra_debugging("=> .4.1: ", @tmp) if($DEBUG > 2);
