...
# }}}
    ...
    debug(0, "=> Number of status counters: $status\n") if($CFG{'DEBUG'} >= 4);
...
sub print_amount_clients {
    if($CFG{'DEBUG'}) {
	debug(0, "=> OID_BASE.totalClients.0\n") if($CFG{'DEBUG'} > 1);
	debug(0, "$OID_BASE.1.0 = $CLIENTS\n");
    }
...
sub print_amount_stats {
    if($CFG{'DEBUG'}) {
	debug(0, "=> OID_BASE.totalStats.0\n") if($CFG{'DEBUG'} > 1);
	debug(0, "$OID_BASE.2.0 = $STATUS\n");
    }
...
sub print_amount_pools {
    if($CFG{'DEBUG'}) {
	debug(0, "=> OID_BASE.totalPools.0\n") if($CFG{'DEBUG'} > 1);
	debug(0, "$OID_BASE.3.0 = $POOLS\n");
    }
...
sub print_amount_medias {
    if($CFG{'DEBUG'}) {
	debug(0, "=> OID_BASE.totalMedias.0\n") if($CFG{'DEBUG'} > 1);
	debug(0, "$OID_BASE.4.0 = $MEDIAS\n");
    }
...
sub print_clients_index {
    ...
    my $success = 0;
    debug(0, "=> OID_BASE.clientTable.clientEntry.IndexClients\n") if($CFG{'DEBUG'} > 1);
	...
	foreach my $key_nr (sort keys %keys_client) {
		...
		debug(0, "=> OID_BASE.clientTable.clientEntry.$key_name.clientName\n") if($CFG{'DEBUG'} > 1);
		
		my $client_nr = 1;
		foreach my $client_name (sort keys %CLIENTS) {
		    if($client_nr == $client_no) {
			debug(0, "$OID_BASE.5.1.$key_nr.$client_nr = $client_nr\n") if($CFG{'DEBUG'});
			
			debug(1, "$OID_BASE.5.1.$key_nr.$client_nr\n");
			debug(1, "integer\n");
			debug(1, "$client_nr\n");
...
# }}}
	...
	my $client_nr = 1;
	foreach my $client_name (sort keys %CLIENTS) {
	    debug(0, "$OID_BASE.5.1.1.$client_nr = $client_nr\n") if($CFG{'DEBUG'});
	    
	    debug(1, "$OID_BASE.5.1.1..$client_nr\n");
	    debug(1, "integer\n");
	    debug(1, "$client_nr\n");
...
sub print_clients_name {
	...
	foreach my $key_nr (sort keys %keys_client) {
		...
		foreach my $client_name (sort keys %CLIENTS) {
			...
			debug(0, "$OID_BASE.5.1.$key_nr.$client_nr = ".$CLIENTS{$client_name}{$key_name}."\n") if($CFG{'DEBUG'});
			...
			debug(1, $CLIENTS{$client_name}{$key_name}."\n");
...
sub print_jobs_names_index {
	...
	debug(0, "=> OID_BASE.jobsNameTable.jobsNameEntry.IndexJobNames\n") if($CFG{'DEBUG'} > 1);
	foreach my $client_name (sort keys %JOBS) {
		...
		foreach my $job_name (sort keys %{ $JOBS{$client_name} }) {
			...
			debug(0, "$OID_BASE.6.1.1.$client_name_num.$job_name_num = $job_name_num\n") if($CFG{'DEBUG'});
			
			debug(1, "$OID_BASE.6.1.1.$client_name_num.$job_name_num\n");
			debug(1, "integer\n");
			debug(1, "$job_name_num\n");
...
# }}}
	...
	my $client_name_num = 1;
	debug(0, "=> OID_BASE.jobsNameTable.jobsNameEntry.clientName.IndexJobNames\n") if($CFG{'DEBUG'} > 1);
	foreach my $client_name (sort keys %JOBS) {
	    my $job_name_num = 1;
	    foreach my $job_name (sort keys %{ $JOBS{$client_name} }) {
		debug(0, "$OID_BASE.6.1.1.$client_name_num.$job_name_num = $job_name_num\n") if($CFG{'DEBUG'});
...
sub print_jobs_names {
	...
	foreach my $key_nr (sort keys %keys_jobs) {
		...
		debug(0, "=> OID_BASE.jobsNameTable.jobsNameEntry.$key_name.clientName.jobName\n") if($CFG{'DEBUG'} > 1);
		...
		foreach my $client_name (sort keys %JOBS) {
			...
			foreach my $job_name (sort keys %{ $JOBS{$client_name} }) {
			    if($job_name_num == $job_no) {
				debug(0, "$OID_BASE.6.1.$key_nr.$client_name_num.$job_name_num = ".$JOBS{$client_name}{$job_name}{$key_name}."\n") if($CFG{'DEBUG'});
				
				debug(1, "$OID_BASE.6.1.$key_nr.$client_name_num.$job_name_num\n");
				if($key_nr == 5) {
				    debug(1, "string\n");
				} else {
				    debug(1, "string\n");
				}
				debug(1, $JOBS{$client_name}{$job_name}{$key_name}."\n");
...
sub print_jobs_ids_index {
	...
	# {{{ Specific client name
	my $client_name_num = 1;
	debug(0, "=> OID_BASE.jobsIDTable.jobsIDEntry.IndexJobIDs\n") if($CFG{'DEBUG'} > 1);
	foreach my $client_name (sort keys %CLIENTS) {
	    if($client_name_num == $CLIENT_NO) {
		my $job_name_num = 1;
		foreach my $job_name (sort keys %{ $STATUS{$client_name} }) {
			...
			foreach my $job_id (sort keys %{ $STATUS{$client_name}{$job_name} }) {
				...
				debug(0, "$OID_BASE.7.1.1.$client_name_num.$job_name_num.$job_id_num = $job_id_num\n") if($CFG{'DEBUG'});
				
				debug(1, "$OID_BASE.7.1.1.$client_name_num.$job_name_num.$job_id_num\n");
				debug(1, "integer\n");
				debug(1, "$job_id_num\n");
...
sub print_jobs_ids {
	...
	# {{{ Specific client name
	my $client_name_num = 1;
	debug(0, "=> OID_BASE.jobsIDTable.jobsIDEntry.clientID.jobName.jobID\n") if($CFG{'DEBUG'} > 1);
	foreach my $client_name (sort keys %CLIENTS) {
	    if($client_name_num == $CLIENT_NO) {
		my $job_name_num = 1;
		debug(0, "=> Client name: '$client_name'\n") if($CFG{'DEBUG'} > 3);
		foreach my $job_name (sort keys %{ $STATUS{$client_name} }) {
		    if($job_name_num == $JOB_NO) {
			my $job_id_num = 1;
			debug(0, "=>   Job name: '$job_name'\n") if($CFG{'DEBUG'} > 3);
			foreach my $job_id (sort keys %{ $STATUS{$client_name}{$job_name} }) {
			    if($job_id_num == $job_name_no) {
				debug(0, "=>     Job ID: '$job_id'\n") if($CFG{'DEBUG'} > 3);
				
				debug(0, "$OID_BASE.7.1.2.$client_name_num.$job_name_num.$job_id_num = $job_id\n") if($CFG{'DEBUG'});
				
				debug(1, "$OID_BASE.7.1.2.$client_name_num.$job_name_num.$job_id_num\n");
				debug(1, "string\n");
				debug(1, "$job_id\n");
...
# }}}
	...
	foreach my $client_name (sort keys %CLIENTS) {
		...
		foreach my $job_id (sort keys %{ $STATUS{$client_name}{$job_name} }) {
		    ...
		    debug(1, "$OID_BASE.7.1.2.$client_name_num.$job_name_num\n");
		    debug(1, "string\n");
		    debug(1, "$job_id\n");
...
sub print_jobs_status_counters_index {
    ...
    debug(0, "=> OID_BASE.statsTable.statsEntry.statsTypeName\n") if($CFG{'DEBUG'} > 1);

    if(defined($key_nr)) {
	# {{{ One specific type name
	my $value = sprintf("%02d", $key_nr+1); # This is the index - offset one!
	if(!$keys_stats{$value}) {
	    debug(0, "=> No such status type ($key_nr)\n") if($CFG{'DEBUG'} > 1);
	    return 0;
	}
...
# }}}
    ...
    # {{{ Output index
    for(; $i <= $max; $i++) {
	debug(0, "$OID_BASE.8.1.1.$i = $i\n") if($CFG{'DEBUG'});

	debug(1, "$OID_BASE.8.1.1.$i\n");
	debug(1, "integer\n");
	debug(1, "$i\n");
...
# }}}
	...
	debug(0, "$OID_BASE.8.1.2.$i = $key_name\n") if($CFG{'DEBUG'});

	debug(1, "$OID_BASE.8.1.2.$i\n");
	debug(1, "string\n");
	debug(1, "$key_name\n");
...
sub print_jobs_status_index {
    ...
    debug(0, "=> OID_BASE.statsTable.statsEntry.IndexStats\n") if($CFG{'DEBUG'} > 1);
	...
	foreach my $client_name (sort keys %CLIENTS) {
		...
		foreach my $job_name (sort keys %{ $STATUS{$client_name} }) {
			...
			foreach my $job_id (sort keys %{ $STATUS{$client_name}{$job_name} }) {
				...
				debug(0, "$OID_BASE.9.1.1.$client_name_num.$job_name_num.$job_id_num = $job_id_num\n") if($CFG{'DEBUG'});
				
				debug(1, "$OID_BASE.9.1.1.$client_name_num.$job_name_num.$job_id_num\n");
				debug(1, "integer\n");
				debug(1, "$job_id_num\n");
...
sub print_jobs_status {
	...
	if(!$CLIENTS[$CLIENT_NO]) {
	    debug(0, "=> No value in this object\n") if($CFG{'DEBUG'} > 1);
	    return 0;
	}
		...
		foreach my $client_name (sort keys %CLIENTS) {
			...
			foreach my $job_name (sort keys %{ $STATUS{$client_name} }) {
				...
				foreach my $job_id (sort keys %{ $STATUS{$client_name}{$job_name} }) {
					...
					debug(0, "=> OID_BASE.statsTable.statsEntry.$key_name.clientId.jobNr\n") if($CFG{'DEBUG'} > 1);

					debug(0, "$OID_BASE.9.1.$key_nr.$client_name_num.$job_name_num.$job_id_num = ".
					      $STATUS{$client_name}{$job_name}{$job_id}{$key_name}."\n") if($CFG{'DEBUG'});
					
					debug(1, "$OID_BASE.9.1.$key_nr.$client_name_num.$job_name_num.$job_id_num\n");
					if(($key_name eq 'start_date') || ($key_name eq 'end_date') || ($key_name eq 'type')) {
					    debug(1, "string\n");
					} else {
					    debug(1, "integer\n");
					}
					debug(1, $STATUS{$client_name}{$job_name}{$job_id}{$key_name}."\n");
...
sub print_pool_index {
    ...
    my($i, $max);
    my $success = 0;
    debug(0, "=> OID_BASE.poolsTable.poolsEntry.IndexPools\n") if($CFG{'DEBUG'} > 1);

    if(defined($pool_no)) {
	# {{{ Specific pool index
	if($pool_no > $POOLS) {
	    debug(0, "=> No value in this object ($pool_no)\n") if($CFG{'DEBUG'});
	    return 0;
	}
...
# }}}
    ...
    # {{{ Output index
    for(; $i <= $max; $i++) {
	debug(0, "$OID_BASE.10.1.1.$i = $i\n") if($CFG{'DEBUG'});

	debug(1, "$OID_BASE.10.1.1.$i\n");
	debug(1, "integer\n");
	debug(1, "$i\n");
...
sub print_pool_names {
    my $pool_no = shift; # Pool number
    my $success = 0;
    debug(0, "=> OID_BASE.poolTable.poolEntry.key.jobid\n") if($CFG{'DEBUG'} > 1);
	...
	foreach my $key_nr (sort keys %keys_pool) {
		...
		foreach my $pool_id (sort keys %POOLS) {
		    if($pool_nr == $pool_no) {
			debug(0, "=> OID_BASE.poolTable.poolEntry->$pool_id.$key_name\n") if($CFG{'DEBUG'} > 1);

			debug(0, "$OID_BASE.10.1.$key_nr.$pool_nr = ".$POOLS{$pool_id}{$key_name}."\n") if($CFG{'DEBUG'});
			
			debug(1, "$OID_BASE.10.1.$key_nr.$pool_nr\n");
			...
			debug(1, $POOLS{$pool_id}{$key_name}."\n");
...
sub print_media_index {
    ...
    my($i, $max);
    my $success = 0;
    debug(0, "=> OID_BASE.mediaTable.mediaEntry.IndexMedia\n") if($CFG{'DEBUG'} > 1);

    if(defined($media_no)) {
	# {{{ Specific media index
	if($media_no > $MEDIAS) {
	    debug(0, "=> No value in this object ($media_no)\n") if($CFG{'DEBUG'});
	    return 0;
	}
...
# }}}
    ...
    # {{{ Output index
    for(; $i <= $max; $i++) {
	debug(0, "$OID_BASE.11.1.1.$i = $i\n") if($CFG{'DEBUG'});

	debug(1, "$OID_BASE.11.1.1.$i\n");
	debug(1, "integer\n");
	debug(1, "$i\n");
...
sub print_media_names {
	...
	foreach my $key_nr (sort keys %keys_media) {
		...
		$key_nr =~ s/^0//;
		debug(0, "=> OID_BASE.mediaTable.mediaEntry.$key_name\n") if($CFG{'DEBUG'} > 1);

		my $media_nr = 1;
		foreach my $media_id (sort keys %MEDIAS) {
		    if($media_nr == $media_no) {
			debug(0, "$OID_BASE.11.1.$key_nr.$media_nr = ".$MEDIAS{$media_id}{$key_name}."\n") if($CFG{'DEBUG'});
			
			debug(1, "$OID_BASE.11.1.$key_nr.$media_nr\n");
			if(($key_name eq 'name') ||
			   ($key_name eq 'type') ||
			...
			debug(1, $MEDIAS{$media_id}{$key_name}."\n");
...
sub write_client_autoprune {
    ...
    debug(0, "=> write_client_autoprune($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
	...
	debug(0, "=> ERROR: Could not prepare SQL query: $dbh->errstr\n");
	return(2);
    }
    debug(0, "=> SQL query prepared: '$QUERY'\n") if($CFG{'DEBUG'} > 3);

    # Execute query
    if(!$sth->execute) {
	debug(0, "Could not execute query: $sth->errstr\n");
	return(2);
    }
    debug(0, "=> SQL query executed\n") if($CFG{'DEBUG'} > 3);
# }}}
...
sub write_client_retentionfile {
    ...
    debug(0, "=> write_client_retentionfile($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_client_retentionjob {
    ...
    debug(0, "=> write_client_retentionjob($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_useonce {
    ...
    debug(0, "=> write_pools_useonce($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_usecatalog {
    ...
    debug(0, "=> write_pools_usecatalog($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_acceptanyvolume {
    ...
    debug(0, "=> write_pools_acceptanyvolume($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_retention {
    ...
    debug(0, "=> write_pools_retention($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_duration {
    ...
    debug(0, "=> write_pools_duration($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_maxjobs {
    ...
    debug(0, "=> write_pools_maxjobs($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_maxfiles {
    ...
    debug(0, "=> write_pools_maxfiles($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_maxbytes {
    ...
    debug(0, "=> write_pools_maxbytes($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_autoprune {
    ...
    debug(0, "=> write_pools_autoprune($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_recycle {
    ...
    debug(0, "=> write_pools_recycle($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_type {
    ...
    debug(0, "=> write_pools_type($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_labelformat {
    ...
    debug(0, "=> write_pools_labelformat($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub write_pools_enabled {
    ...
    debug(0, "=> write_pools_enabled($arg_sub, $arg_type, $arg_val)\n") if($CFG{'DEBUG'} > 3);
...
sub call_write {
    ...
    debug(0, "=> call_write($func_base, $func_sub, $func_type, $func_arg)\n") if($CFG{'DEBUG'} > 3);
    my $func = $writables{$OID_BASE.".".$func_base};
    return 1 if(!defined($func));

    $function = "write_".$func;
    debug(0, "=> Calling function '$function($func_sub, $func_type, $func_arg)'\n") if($CFG{'DEBUG'} > 2);
...
# }}} # Next value
			...
			debug(0, ">> No OID at that level (-2) - get next branch OID: $next1$next2.$next3\n") if($CFG{'DEBUG'} >= 2);
			if(!&call_print($next1, $next3)) {
...
# }}} # Next value
			    ...
			    debug(0, ">> No OID at that level (-3) - get next branch OID: $next1$next2.$next3\n") if($CFG{'DEBUG'} >= 2);
			    if(!&call_print($next1, $next3)) {
...
# }}} # Next value
				...
				debug(0, ">> No OID at that level (-4) - get next branch OID: $next1$next2.$next3\n") if($CFG{'DEBUG'} >= 2);
				&call_print($next1, $next3);
			    }
# }}} # Call functions -> -3
			}
...
# }}} # Get _this_ OID
	    ...
	    debug(0, "=> Type: '$type', Value: '$value'\n") if($CFG{'DEBUG'} > 3);

	    my $code = &call_write($tmp[0].".1.".$tmp[2], $tmp[3], $type, $value);
	    if($code == 0) {
		debug(0, "=> Successfully modified object\n") if($CFG{'DEBUG'} > 2);
		debug(1, "\n");
	    } elsif($code == 1) {
		debug(0, "=> ERROR: Object not writable\n");
		debug(1, "not-writable\n");
	    } elsif($code == 2) {
		debug(0, "=> ERROR: Input of wrong type (='$type')\n");
		debug(1, "wrong-type\n");
	    } else {
		debug(0, "=> ERROR: Generic failure\n");
	    }
